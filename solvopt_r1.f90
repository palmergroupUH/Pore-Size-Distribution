subroutine solvopt(n,x,f,fun,flg,grad,flfc,func,flgc,gradc)
      ...
      double precision g[allocatable](:)
      double precision g0[allocatable](:)
      double precision g1[allocatable](:)
      double precision gt[allocatable](:)
      double precision gc[allocatable](:)
      double precision z [allocatable](:)
      double precision x1[allocatable](:)
      double precision xopt[allocatable](:)
      double precision xrec[allocatable](:)
      double precision grec[allocatable](:)
      double precision xx[allocatable](:)
      double precision deltax[allocatable](:)
      integer idx[allocatable](:)
       ...
       if (termflag) then
           ...
           if (ii.eq.0 .or. stopping)  then
                ...
                elseif(dabs(f-frec).gt.options(3)*dabs(f) .and. d.lt.options(2)*nx .and. constr) then
                   continue
                elseif  (dabs(f-fopt).le.options(3)*dabs(f) .or. dabs(f).le.lowfbound .or. (dabs(f-fopt).le.options(3).and. termx.ge.limxterm )) then
                  if (stopf) then
